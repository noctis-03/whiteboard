...
function commitFreehandStroke(pts){
  ...
  if(spec.kind==='taper-path'){
    ...
    finalEl && setAttrs(finalEl, spec.attrs);
  }
...
function cancelSingleFingerActions(){
  ...
  if(touchLasso){ touchLasso=null; hideSelRect(); }
...
function addCardWindow(opts={}){
  ...
  closeBtn.addEventListener('click',e=>{ e.stopPropagation(); el.remove(); updateMinimap(); });
...
/* ════════════════════════════════════════════════════════
   STARTUP WINDOWS (announcement / system windows)
   — data-win-role="startup" distinguishes them from
     user-created card windows (data-kind="card" .el)
════════════════════════════════════════════════════════ */
...
function addStartupWindow(opts={}){
  ...
  head.innerHTML=`<div class="start-window-title">${opts.title??'창'}</div><button type="button" class="start-window-close">✕</button>`;
  const content=document.createElement('div');
  content.className='start-window-content';
  bindWindowContentGuards(content);
  const closeBtn=head.querySelector('.start-window-close');
  closeBtn.addEventListener('click',e=>{ e.stopPropagation(); el.remove(); });
  ...
  head.addEventListener('touchstart',e=>{ if(e.touches.length!==1) return; e.preventDefault(); e.stopPropagation(); beginStartupDrag(el,e.touches[0].clientX,e.touches[0].clientY); },{passive:false});
  ...
  items.forEach(item=>{
    ...
    row.innerHTML=`
      ...
      <div class="recent-actions">
        <button type="button" class="launch-btn small primary">열기</button>
        <button type="button" class="launch-btn small ghost">삭제</button>
      </div>`;
    const [openBtn, delBtn]=row.querySelectorAll('button');
    openBtn.addEventListener('click',()=>openRecentBoard(item.id));
    delBtn.addEventListener('click',()=>removeRecentBoard(item.id));
...
function createLauncherWindow(){
  return addStartupWindow({
    ...
    render(content){
      content.innerHTML=`
        <div class="start-window-note">새 보드를 시작하거나 저장한 캔버스 JSON 파일을 바로 불러올 수 있습니다.</div>
        <div class="start-window-actions">
          <button type="button" class="launch-btn primary" id="launcher-new-btn">새 캔버스</button>
          <button type="button" class="launch-btn" id="launcher-load-btn">캔버스 불러오기</button>
        </div>
        <div class="recent-meta">창 상단을 드래그하면 이동되고, ✕ 버튼으로 닫을 수 있습니다.</div>`;
      content.querySelector('#launcher-new-btn').addEventListener('click',createFreshBoard);
      content.querySelector('#launcher-load-btn').addEventListener('click',()=>document.getElementById('load-in').click());
    }
  });
}
function createRecentBoardsWindow(){
  return addStartupWindow({
    ...
    render(content){
      content.innerHTML=`
        <div class="start-window-note">이 브라우저에서 저장하거나 불러온 최근 보드가 여기에 표시됩니다.</div>
        <div class="recent-toolbar">
          <button type="button" class="launch-btn small" id="recent-save-btn">현재 캔버스 저장</button>
        </div>
        <div class="recent-list" id="recent-board-list"></div>`;
      content.querySelector('#recent-save-btn').addEventListener('click',saveBoard);
      renderRecentBoards();
    }
  });
}
function createShortcutsWindow(){
  return addStartupWindow({
    ...
    render(content){
      ...
      content.innerHTML=`
        <div class="start-window-note">자주 쓰는 키를 빠르게 확인할 수 있는 요약 창입니다.</div>
        <div class="shortcut-list">${rows.map(([key,desc])=>`<div class="shortcut-row"><div class="shortcut-key">${key}</div><div class="shortcut-desc">${desc}</div></div>`).join('')}</div>`;
    }
  });
}
